/*
 * FS-UAE — external JSON-RPC control surface ("Claude RPC")
 *
 * Enables external tools (Claude via MCP server, scripts, CI harnesses)
 * to control a running FS-UAE instance over HTTP/JSON.  Wraps the
 * existing internal `debug.cpp` API surface (activate_debugger,
 * deactivate_debugger, get_long_debug, returnregx, save_state).
 *
 * Activation:
 *   FSUAE_RPC_PORT=8765 fs-uae <config>
 *
 * If the env var is not set, this module is a no-op (claude_rpc_init
 * returns immediately).  The HTTP server runs in its own worker thread;
 * memory / register reads are SAFE while the emulator is PAUSED (i.e.
 * after a /v1/pause call).  Reading while the emulator is running may
 * race; clients should pause first for stable snapshots.
 *
 * v1 endpoints:
 ...
 *
 * Conventions:
 *   - addr/len query params accept decimal or 0x-prefixed hex
...
extern "C" void claude_rpc_init(void);
...
static void ep_ping(int fd) {
    send_response(fd, 200, "{\"ok\":true,\"service\":\"fs-uae claude-rpc v1\"}\n");
}
...
static void ep_state_save(int fd, const char *qs) {
    ...
    int rc = save_state(path_buf, "claude-rpc save");
    if (rc) {
        char body[1280];
...
static void *rpc_worker(void *arg) {
    ...
    if (srv < 0) {
        fprintf(stderr, "[claude-rpc] socket() failed: %s\n", strerror(errno));
        return NULL;
    }
    int one = 1;
    ...
    if (bind(srv, (struct sockaddr *)&sa, sizeof sa) < 0) {
        fprintf(stderr, "[claude-rpc] bind(127.0.0.1:%d) failed: %s\n",
            port, strerror(errno));
        close(srv);
        return NULL;
    }
    if (listen(srv, 8) < 0) {
        fprintf(stderr, "[claude-rpc] listen() failed: %s\n", strerror(errno));
        close(srv);
        return NULL;
    }
    fprintf(stderr, "[claude-rpc] listening on http://127.0.0.1:%d/\n", port);
    for (;;) {
        int cfd = accept(srv, NULL, NULL);
        if (cfd < 0) {
            if (errno == EINTR) continue;
            fprintf(stderr, "[claude-rpc] accept() failed: %s\n", strerror(errno));
            break;
        }
        handle_request(cfd);
    }
...
extern "C" void claude_rpc_init(void) {
    ...
    if (port <= 0 || port > 65535) {
        fprintf(stderr, "[claude-rpc] invalid FSUAE_RPC_PORT='%s'\n", env);
        return;
    }
    pthread_t tid;
    if (pthread_create(&tid, NULL, rpc_worker, (void *)(intptr_t)port) != 0) {
        fprintf(stderr, "[claude-rpc] pthread_create failed: %s\n",
            strerror(errno));
        return;
    }
    pthread_detach(tid);
