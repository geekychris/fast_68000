...
static void ep_resume(int fd) {
    ...
    for (int i = 0; i < BREAKPOINT_TOTAL; i++) {
        if (bpnodes[i].enabled) { claude_rpc_arm_bp_skip(); break; }
    }
 ...
 *                          check in claude_rpc_trace_bp_hook)
...
static void ep_bp_add(int fd, const char *qs) {
     ...
     * claude_rpc_trace_bp_hook above.  Requires an output file via the
