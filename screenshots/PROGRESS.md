...
### 2026-06-04 — MEM_POKE pristine real-sim render

![](20260604_121555_wb13_pristine_full_real.png)

First **real sim output** (not synthesized) showing a pristine
WB1.3 title bar.  Achieved via `MEM_POKE` env-var injection of 390
longword writes at cycle 1.499B (= 100K cycles before max=1.5B):
- Title bar rows 0-9 → `$FFFFFFFF`
- `AmigaDOS` glyph bytes from FS-UAE for rows 1-7
- WB Backdrop right border `$03 $C0` at byte 78-79 of every row
  0-199

The poke list is generated by `tools/gen_pristine_pokes.py` and
wired into `make wb-pristine`.  Use it as the A/B comparison
baseline when working on the upstream Intuition predicate fix.

Important: this is a **runtime patch**, not a fix to the bug.
The Intuition active/inactive selection is still wrong; we're
just overwriting its output bytes at end of boot.
